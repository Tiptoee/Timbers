...
public class TreeEntity : Tree, IInteractableObject, IDamageable, IMinimapObject
{
    ...
    public override void Initialize()
    {
        ...
        if (!this.Fallen)
            MinimapManager.AddOrUpdateMinimapObject(this);
    }
    ...
    public GameActionPack TryApplyDamage(GameActionPack pack, INetObject damager, float amount, InteractionContext context, Item tool, Type damageDealer = null, float experienceMultiplier = 1f)
    {
        // if the tree is really young, just outright uproot and destroy it.
        if (this.GrowthPercent < this.SaplingGrowthPercent) return this.TryKillSapling(pack, damager, amount, context);
    ...
    private GameActionPack TryKillSapling(GameActionPack pack, INetObject damager, float amount, InteractionContext context)
    {
        ...
        pack.AddPostEffect(() => EcoSim.PlantSim.DestroyPlant(this, DeathType.Harvesting));
        return pack;
    }
